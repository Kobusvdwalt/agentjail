...
| `src/agentjail/api/routes/exec.py` | REST routes: exec (direct), shell (via /bin/sh -c). |
...
| `src/agentjail/mcp/server.py` | FastMCP tool definitions (14 tools). Uses global `_manager`. |
