...
class TestSymlinkEscape:
    ...
    async def test_symlink_read_state_file(self, client: httpx.AsyncClient, sandbox: dict):
        await shell(client, sandbox["id"], "ln -s /var/lib/agentjail/state.json /home/statelink")
    ...
    async def test_symlink_to_sandbox_base_dir(self, client: httpx.AsyncClient, sandbox: dict):
        await shell(client, sandbox["id"], "ln -s /var/lib/agentjail/sandboxes /home/sandboxes")
...
class TestShellEscape:
    async def test_shell_hostname_is_isolated(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "hostname")
        hostname = result["stdout"].strip()
        assert len(hostname) > 0

    async def test_shell_cannot_read_state_file(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "cat /var/lib/agentjail/state.json")
        assert result["exit_code"] != 0 or result["stdout"].strip() == ""

    async def test_shell_cannot_read_sandbox_base_dir(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "ls /var/lib/agentjail/sandboxes/")
        assert result["exit_code"] != 0

    async def test_shell_cannot_access_docker_socket(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "ls /var/run/docker.sock")
        assert result["exit_code"] != 0

    async def test_shell_cannot_read_proc_environ(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "cat /proc/1/environ 2>/dev/null")
        stdout = result["stdout"]
        assert "AGENTJAIL" not in stdout

    async def test_shell_env_does_not_leak_host_vars(self, client: httpx.AsyncClient, sandbox: dict):
...
class TestReadOnlyMounts:
    ...
    async def test_can_write_to_home(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "echo ok > /home/test.txt && cat /home/test.txt")
        assert result["exit_code"] == 0
        assert "ok" in result["stdout"]

    async def test_can_write_to_tmp(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "echo tmp > /tmp/test.txt && cat /tmp/test.txt")
        assert result["exit_code"] == 0
        assert "tmp" in result["stdout"]

    async def test_tmp_does_not_persist_across_exec(self, client: httpx.AsyncClient, sandbox: dict):
...
class TestProcessIsolation:
    async def test_pid_namespace_isolation(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "cat /proc/self/status | grep -i '^pid:'")
        assert result["exit_code"] == 0

    async def test_cannot_see_host_processes(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "ls /proc/ | grep -E '^[0-9]+$' | wc -l")
        if result["exit_code"] == 0:
            count = int(result["stdout"].strip())
            assert count < 10

    async def test_cannot_signal_host_processes(self, client: httpx.AsyncClient, sandbox: dict):
    ...
    async def test_pids_limit_enforcement(self, client: httpx.AsyncClient):
        ...
        try:
            result = await shell(
                client, sb["id"], "for i in $(seq 1 20); do sleep 10 & done; wait 2>&1; echo done"
            )
            assert "Resource temporarily unavailable" in result["stderr"] or result["exit_code"] != 0 or "done" in result["stdout"]
        finally:
...
class TestNetworkIsolation:
    async def test_network_disabled_by_default(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "ping -c 1 -W 1 8.8.8.8 2>&1 || echo NETWORK_BLOCKED")
        assert "NETWORK_BLOCKED" in result["stdout"] or result["exit_code"] != 0

    async def test_network_disabled_no_dns(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "getent hosts google.com 2>&1 || echo DNS_BLOCKED")
        assert "DNS_BLOCKED" in result["stdout"] or result["exit_code"] != 0

    async def test_network_disabled_no_loopback(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(
            client,
            sandbox["id"],
    ...
    @pytest.mark.network
    async def test_network_enabled_can_reach_external(self, client: httpx.AsyncClient, sandbox_with_network: dict):
        result = await shell(
            client,
...
class TestResourceExhaustion:
    ...
    async def test_memory_exhaustion_contained(self, client: httpx.AsyncClient):
        ...
        try:
            result = await shell(
                client, sb["id"], "python3 -c 'x = bytearray(200 * 1024 * 1024)'", timeout=10
            )
            assert result["exit_code"] != 0
        finally:
            await remove_sandbox(client, sb["id"])

    async def test_disk_exhaustion_contained(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "dd if=/dev/zero of=/home/big bs=1M count=100 2>&1")
    ...
    async def test_file_descriptor_exhaustion(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(
            client,
            sandbox["id"],
    ...
    async def test_tmp_size_limit(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "dd if=/dev/zero of=/tmp/fill bs=1M count=100 2>&1")
...
class TestCrossSandboxIsolation:
    async def test_sandbox_a_cannot_read_sandbox_b_files(self, client: httpx.AsyncClient):
    ...
    async def test_sandbox_a_cannot_access_sandbox_b_via_path(self, client: httpx.AsyncClient):
        sb_a = await create_sandbox(client)
        sb_b = await create_sandbox(client)
        try:
            a_id = sb_a["id"]
...
class TestProcAbuse:
    async def test_proc_is_read_only(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "echo x > /proc/sys/kernel/hostname 2>&1")
        assert result["exit_code"] != 0

    async def test_proc_self_shows_sandboxed_process(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "cat /proc/self/status | grep -i '^pid:'")
        assert result["exit_code"] == 0

    async def test_cannot_read_host_proc_1_cmdline(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "cat /proc/1/cmdline 2>/dev/null || echo BLOCKED")
        stdout = result["stdout"]
        assert "uvicorn" not in stdout
        assert "agentjail" not in stdout

    async def test_proc_net_shows_isolated_interfaces(self, client: httpx.AsyncClient, sandbox: dict):
...
class TestPrivilegeEscalation:
    async def test_cannot_mount_filesystem(self, client: httpx.AsyncClient, sandbox: dict):
    ...
    async def test_cannot_use_chroot(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "chroot / /bin/sh -c 'echo escaped' 2>&1")
        assert result["exit_code"] != 0
    ...
    async def test_runs_as_unprivileged_user(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "id")
        assert result["exit_code"] == 0
        assert "uid=1000" in result["stdout"]
        assert "gid=1000" in result["stdout"]

    async def test_cannot_create_device_nodes(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "mknod /home/evil c 1 3 2>&1")
        assert result["exit_code"] != 0

    async def test_cannot_change_file_owner(self, client: httpx.AsyncClient, sandbox: dict):
...
class TestEnvLeakage:
    async def test_no_host_env_leak_agentjail_vars(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "env")
        for line in result["stdout"].splitlines():
            assert not line.startswith("AGENTJAIL_")

    async def test_no_host_env_leak_service_vars(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "env")
        stdout = result["stdout"]
    ...
    async def test_only_expected_env_vars(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "env")
        allowed_prefixes = ("PATH=", "HOME=", "TERM=", "SHLVL=", "PWD=", "_=")
        for line in result["stdout"].splitlines():
            if "=" in line:
                assert any(line.startswith(p) for p in allowed_prefixes), f"Unexpected env var: {line}"
    ...
    async def test_per_exec_env_override(self, client: httpx.AsyncClient):
        ...
        try:
            result = await exec_cmd(
                client, sb["id"], "/bin/sh", args=["-c", "echo $X"], env={"X": "override"}
            )
