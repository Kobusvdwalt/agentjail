...
class TestShell:
    async def test_shell_echo(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "echo hello")
        assert result["exit_code"] == 0
        assert "hello" in result["stdout"]

    async def test_shell_exit_code(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "exit 7")
        assert result["exit_code"] == 7

    async def test_shell_stderr(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "echo err >&2")
        assert "err" in result["stderr"]

    async def test_shell_pipe(self, client: httpx.AsyncClient, sandbox: dict):
        result = await shell(client, sandbox["id"], "echo hello world | wc -w")
        assert result["stdout"].strip() == "2"

    async def test_shell_env_from_config(self, client: httpx.AsyncClient):
